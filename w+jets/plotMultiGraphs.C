...
void plotMultiGraphs(){
  ...
  for(int i=0; i<3; i++){
	...
	float mean[11],meanerr[11],RMS[11],RMSerr[11];
	float rel[11], relerr[11];
	float masserr[11];
	...
	graph_mean[j] = new TGraphErrors(11,mass,mean,masserr,meanerr);
	...
	graph_RMS[j] = new TGraphErrors(11,mass,RMS,masserr,RMSerr);
	...
	graph_RMSMean[j] = new TGraphErrors(11,mass,rel,masserr,relerr);
