...
void xAna_hh_massResolution(std::string inputFile, bool matchb=true, bool debug=false, bool cut=true){
  ...
  TH1F* h_mass     = new TH1F("h_mass","",35,40,110);
  ...
  for(int i=0; i<nHistos; i++)
    {
      ...
      h_diff_SD[i]->SetXTitle("Raw Puppi+Softdrop (m-80)/80"); //change to 80

      h_diff_SDCorr[i] = (TH1F*)h_massDiff->Clone(Form("h_diff_SDCorr_%s",prefix[i].data()));
      h_diff_SDCorr[i]->SetXTitle("L2L3-corrected Puppi+Softdrop (m-80)/80");//change to 80

      h_diff_SDCorrThea[i] = (TH1F*)h_massDiff->Clone(Form("h_diff_SDCorrThea_%s",prefix[i].data()));
      h_diff_SDCorrThea[i]->SetXTitle("Thea-corrected Puppi+Softdrop (m-80)/80");//change to 80

      h_diff_PR[i] = (TH1F*)h_massDiff->Clone(Form("h_diff_PR_%s",prefix[i].data()));
      h_diff_PR[i]->SetXTitle("Raw CHS+Pruned (m-80)/80");//change to 80

      h_diff_PRCorr[i] = (TH1F*)h_massDiff->Clone(Form("h_diff_PRCorr_%s",prefix[i].data()));
      h_diff_PRCorr[i]->SetXTitle("L2L3-corrected CHS+Pruned (m-80)/80");//change to 80
  ...
  for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
    ...
    for(int ig=0; ig < nGenPar; ig++){
      ...
      if(abs(genParId[ig])!=24)continue; //change to 24?
      
      int da1_pid = abs(genDa1[ig]);
      int da2_pid = abs(genDa2[ig]);

      if(da1_pid > 10 && da1_pid < 19)continue;
      if(da2_pid > 10 && da2_pid < 19)continue;
	...
	h_diff_SD[i]->Fill((fatjetSDmass[jet]-80)/80); //change to 80
	h_diff_SDCorr[i]->Fill((fatjetSDmassL2L3Corr[jet]-80)/80);//change to 80
	h_diff_SDCorrThea[i]->Fill((thea_mass-80)/80);//change to 80
	h_diff_PR[i]->Fill((fatjetPRmass[jet]-80)/80);//change to 80
	h_diff_PRCorr[i]->Fill((fatjetPRmassL2L3Corr[jet]-80)/80);//change to 80

	h_diff_SD[2]->Fill((fatjetSDmass[jet]-80)/80);//change to 80
	h_diff_SDCorr[2]->Fill((fatjetSDmassL2L3Corr[jet]-80)/80);//change to 80
	h_diff_SDCorrThea[2]->Fill((thea_mass-80)/80);//change to 80
	h_diff_PR[2]->Fill((fatjetPRmass[jet]-80)/80);//change to 80
	h_diff_PRCorr[2]->Fill((fatjetPRmassL2L3Corr[jet]-80)/80);//change to 80
