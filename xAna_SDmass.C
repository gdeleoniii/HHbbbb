...
void xAna_hh(std::string inputFile,char name) {
  ...
  for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
    ...
    Int_t lead = fatty[0];
    Int_t sublead = fatty[1];

    h_leadjet->Fill(fatjetSDmass[lead]);
    h_subleadjet->Fill(fatjetSDmass[sublead]);
  ...
  h_leadjet->SetLineColor(kRed+2);
  h_leadjet->Draw();
  h_leadjet->SetStats(0);
  h_leadjet->SetLineWidth(2);
  h_leadjet->SetTitle("SD Mass");
  h_subleadjet->SetLineWidth(2);
  h_subleadjet->SetLineColor(kBlue+2);
  h_subleadjet->Draw("same");  

  TLegend *legend = new TLegend(0.6,0.12,0.9,0.3);
  legend->AddEntry(h_leadjet,"Leading Jet","l");
  legend->AddEntry(h_subleadjet,"Subleading Jet", "l");
  legend->SetFillColor(0);
  legend->Draw();

  TFile* outfile = new TFile(Form("SD_%d.root",name),"recreate");
