...
private:
  void analyze( const Event & iEvent, const EventSetup & iSetup ) {
    ...
    int cut_boson = 0;
    for(int i=0;i<2;i++) {
	if(l4_B[i].Pt() < 30)continue;
	if(fabs(l4_B[i].Eta()) > 2.5)continue;	
	float deta_C = fabs(l4_B[0].Eta() - l4_B[1].Eta());
	if(deta_C > 1.3)continue;
	
	//counter_numerator++;

	h_C1[i]->Fill(l4_B[i].Pt(),weight);
	h_C2[i]->Fill(l4_B[i].Pz(),weight);
	h_C3[i]->Fill(l4_B[i].M(),weight);
	h_C4[i]->Fill(l4_B[i].Rapidity(),weight);
	cut_boson++;
    }
    if(cut_boson>=2)counter_numerator++;
