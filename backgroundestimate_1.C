...
void passfailratio(std::string inputFile, std::string name) {
  ...
  for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
    ...
    if(addjet_doubleSV[addJetIndex[0]]>0.6 && addjet_doubleSV[addJetIndex[1]]<0.6) {
      ...
      h_leadPR_1->Fill(fatjetPRmass[aa]);
      h_sublPR_1->Fill(fatjetPRmass[ee]);
    ...
    //two anti-tag
      ...
      h_leadPR_2->Fill(fatjetPRmass[aa],weight);
      h_sublPR_2->Fill(fatjetPRmass[ee],weight);
  ...
  TFile* outfile = new TFile(Form("%s_bkgest.3.1.root",name.data()),"recreate");
